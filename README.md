...
PyQt6 と Plotly を使ったグラフ描画アプリです。Bluetooth Low Energy（BLE）経由でデバイスから受信したデータをリアルタイムで折れ線グラフに表示し、受信ログを CSV ファイルに自動保存します。保存済みの CSV ログからもグラフを再表示できます。
...
3. 読み込みが成功すると、グラフが更新されます。

#### 読み込み対象のデータ

本アプリが保存した CSV 形式（[ログファイル形式（CSV）](#ログファイル形式csv)）を読み込みます。

| 列名 | グラフでの用途 |
|------|---------------|
| `sample_index` | 横軸（サンプル番号）。空の場合は連番を使用 |
| `parsed_value` | 縦軸（値）。空または数値に変換できない行はスキップ |

`raw_data` や `received_at` はグラフ描画には使いませんが、CSV に含まれていても問題ありません。

#### 表示内容

- グラフタイトル: `ファイル名 (プロット件数/総行数 件)` の形式
  - 例: `log_20260618_120000_AA-BB-CC-DD-EE-FF.csv (10/12 件)`
- ログ表示ラベル: `表示中: （ファイルパス）`
...
│ ログ: 記録中 / 保存済み / 表示中 のファイル名 [CSVからグラフ] │
├─────────────────────────────────────────────────────────┤
│                                                         │
│              Plotly 折れ線グラフ                        │
│                                                         │
