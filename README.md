...
- 初回データ受信後、グラフ上部に **13 系列のチェックボックス**（`turn`, `speed`, `battery`, … `roll`, `yaw`, `pitch`）が表示されます。初期状態では `gyro`, `bright`, `roll`, `yaw`, `pitch` が ON です。
...
Bluetooth 接続中に自動保存される CSV は、LogAnalyzer 2015 等と互換の **10 データ列** に、IMU 姿勢角 **3 列**（`roll`, `yaw`, `pitch`）を追加した形式です。`#` で始まる行はコメントとして扱われ、データ行はカンマ区切りの数値列です。本アプリが保存するファイルでは、コメント行は列名の 1 行のみです。

##### ファイル構成

| 行 | 内容 |
|----|------|
| 1 行目 | `# time, turn, speed, battery, angleL, angleR, bright, gyro, Kp, Ki, Kd, roll, yaw, pitch` — データ列名 |
| 2 行目以降 | データ行（ヘッダー行はありません） |

LogAnalyzer 2015 等が出力した既存ログ（10 データ列のみ）や、追加コメント行付きの CSV も **CSVからグラフ** で読み込めます。姿勢角 3 列がない行では `roll` / `yaw` / `pitch` は null として扱われます。
...
| `speed` | 速度 |
| `battery` | バッテリー電圧 |
| `angleL` | 左モーター角度（Pybricks サンプルではカラーセンサー H） |
| `angleR` | 右モーター角度（Pybricks サンプルではカラーセンサー S） |
| `bright` | 明るさ（Pybricks サンプルではカラーセンサー V） |
| `gyro` | ジャイロ角速度（deg/s） |
| `Kp`, `Ki`, `Kd` | PID パラメータ |
| `roll` | ロール角（deg） |
| `yaw` | ヨー角（deg） |
...
| 条件 | 挙動 |
|------|------|
| 列数が 13 未満 | 不足列を null でパディング |
| 数値に変換できない列 | その列のみ null |
| 14 列以上で先頭列が数値 | 先頭を `time`、続く 13 列をデータ列として解釈 |
| 13 列以下で先頭からデータ列 | `time` なし。接続開始からの経過 ms を使用 |

CSV への記録では null は **空欄** として出力します。**CSVからグラフ** で読み込む際も空欄は null として解釈されます。

##### 記録例（Bluetooth 受信時の自動保存）

完全な 14 列（`time` + 13 データ列）:

```csv
# time, turn, speed, battery, angleL, angleR, bright, gyro, Kp, Ki, Kd, roll, yaw, pitch
315,0.000000,0.000000,7971,120.000000,80.000000,0.090000,44.000000,0.000000,0.000000,0.000000,2.500000,-12.000000,1.250000
318,0.000000,0.000000,7970,121.000000,81.000000,0.100000,46.000000,0.000000,0.000000,0.000000,2.600000,-11.800000,1.300000
```

旧形式（10 データ列のみ、`time` 付き 11 列）も受信・読み込み可能:
...
- 本アプリが Bluetooth 受信時に自動保存した CSV（13 データ列 + 姿勢角 3 列）
- LogAnalyzer 2015 等の既存ログ（10 データ列）
- `#` で始まる行はコメントとして無視
- ヘッダー例: `# time, turn, speed, battery, angleL, angleR, bright, gyro, Kp, Ki, Kd, roll, yaw, pitch`
- 10 データ列のみの旧ヘッダーでも読み込み可能
...
```csv
# time, turn, speed, battery, angleL, angleR, bright, gyro, Kp, Ki, Kd, roll, yaw, pitch
315,0,0,7971,120,80,0.09,44,0,0,0,2.5,-12.0,1.25
```

LogAnalyzer 2015 等の既存ログ例:
...
| 状況 | 初期表示 |
|------|---------|
| Bluetooth リアルタイム受信 | `gyro`, `bright`, `roll`, `yaw`, `pitch` が ON |
| 本アプリ形式 / 旧10列 CSV 読み込み | ヘッダーに存在する系列のうち、上記 5 系列が ON |
...
- レガシー形式の CSV は系列数が多いため、必要な系列だけを ON にして表示することを推奨します（姿勢角は `roll`, `yaw`, `pitch` チェックボックスで個別に切り替え可能）。
...
| 形式 | 列数 | 例 | 説明 |
|------|------|-----|------|
| データ列のみ | 1〜13 列 | `-45,10,7971,0,0,0.09,44,0,0,0,2.5,-12,1.25` | 先頭から `turn` … `pitch` に順に対応。不足列は null |
| time + データ列 | 14 列 | `315,-45,10,7971,0,0,0.09,44,0,0,0,2.5,-12,1.25` | 先頭が `time`（ms）、続く 13 列がデータ列 |
| 旧10列（後方互換） | 10〜11 列 | `315,-45,10,7971,0,0,0.09,44,0,0,0` | `roll` / `yaw` / `pitch` は null |

列の対応順:

```
turn, speed, battery, angleL, angleR, bright, gyro, Kp, Ki, Kd, roll, yaw, pitch
```
...
- 文字コード: UTF-8
- 区切り: 1 レコードごとに改行（`\n` 推奨）を付ける
- 内容: [受信データ形式](#受信データ形式) に準拠（13 データ列、または `time` 付き 14 列）

送信例:

```text
315,0,0,7971,120,80,0.09,44,0,0,0,2.5,-12.0,1.25
318,0,0,7970,121,81,0.10,46,0,0,0,2.6,-11.8,1.3
```
...
| 列 | Pybricks API | 内容 |
|----|--------------|------|
| `gyro` | `hub.imu.angular_velocity(Axis.Z)` | ヨー角速度（deg/s） |
| `roll`, `pitch` | `hub.imu.tilt()` | ロール角・ピッチ角（deg） |
| `yaw` | `hub.imu.heading()` | ヨー角（deg、プログラム開始時を 0 とする累積値） |
| `angleL`, `angleR`, `bright` | `color_sensor.hsv()` | カラーセンサーの H, S, V 生値 |
| `battery` | `hub.battery.voltage()` | バッテリー電圧（mV） |

注意: `hub.imu.orientation()` は 3×3 の回転行列を返すため、姿勢角の取得には `tilt()` と `heading()` を使います。
...
カラーセンサーは `hsv()` の生値を `angleL=h`, `angleR=s`, `bright=v` 列にそのまま送ります。`turn`, `speed`, `Kp`, `Ki`, `Kd` はサンプルでは 0 固定です。
...
│ [turn] [speed] [battery] [gyro] [roll] [yaw] [pitch] ...  ← 系列チェックボックス   │
├────────────────────────────────────────────────────────────────────────────┤
│                                                                            │
│              Plotly 折れ線グラフ（凡例付き）                               │
│                                                                            │
