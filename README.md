...
- 初回データ受信後、グラフ上部に **12 系列のチェックボックス**（`turn`, `speed`, `battery`, … `roll`, `yaw`, `pitch`）が表示されます。初期状態では `bright`, `roll`, `yaw`, `pitch` が ON です。
...
Bluetooth 接続中に自動保存される CSV は、LogAnalyzer 2015 等と互換の **9 データ列**（`gyro` を除く）に、IMU 姿勢角 **3 列**（`roll`, `yaw`, `pitch`）を追加した形式です。`#` で始まる行はコメントとして扱われ、データ行はカンマ区切りの数値列です。本アプリが保存するファイルでは、コメント行は列名の 1 行のみです。

##### ファイル構成

| 行 | 内容 |
|----|------|
| 1 行目 | `# time, turn, speed, battery, angleL, angleR, bright, Kp, Ki, Kd, roll, yaw, pitch` — データ列名 |
...
| 条件 | 挙動 |
|------|------|
| 列数が 12 未満 | 不足列を null でパディング |
| 数値に変換できない列 | その列のみ null |
| 13 列以上で先頭列が数値 | 先頭を `time`、続く 12 列をデータ列として解釈 |
| 12 列以下で先頭からデータ列 | `time` なし。接続開始からの経過 ms を使用 |

CSV への記録では null は **空欄** として出力します。**CSVからグラフ** で読み込む際も空欄は null として解釈されます。

##### 記録例（Bluetooth 受信時の自動保存）

完全な 13 列（`time` + 12 データ列）:

```csv
# time, turn, speed, battery, angleL, angleR, bright, Kp, Ki, Kd, roll, yaw, pitch
315,0.000000,0.000000,7971,120.000000,80.000000,0.090000,0.000000,0.000000,0.000000,2.500000,-12.000000,1.250000
318,0.000000,0.000000,7970,121.000000,81.000000,0.100000,0.000000,0.000000,0.000000,2.600000,-11.800000,1.300000
```
...
- 本アプリが Bluetooth 受信時に自動保存した CSV（12 データ列）
- LogAnalyzer 2015 等の既存ログ（`gyro` 列を含む場合あり）
- `#` で始まる行はコメントとして無視
- ヘッダー例: `# time, turn, speed, battery, angleL, angleR, bright, Kp, Ki, Kd, roll, yaw, pitch`
...
```csv
# time, turn, speed, battery, angleL, angleR, bright, Kp, Ki, Kd, roll, yaw, pitch
315,0,0,7971,120,80,0.09,0,0,0,2.5,-12.0,1.25
```
...
| 状況 | 初期表示 |
|------|---------|
| Bluetooth リアルタイム受信 | `bright`, `roll`, `yaw`, `pitch` が ON |
| 本アプリ形式 / 旧 CSV 読み込み | ヘッダーに存在する系列のうち、`bright`, `roll`, `yaw`, `pitch` が ON |
...
- レガシー形式の CSV は系列数が多いため、必要な系列だけを ON にして表示することを推奨します（姿勢角は `roll`, `yaw`, `pitch` で確認）。
...
| 形式 | 列数 | 例 | 説明 |
|------|------|-----|------|
| データ列のみ | 1〜12 列 | `-45,10,7971,0,0,0.09,0,0,0,2.5,-12,1.25` | 先頭から `turn` … `pitch` に順に対応。不足列は null |
| time + データ列 | 13 列 | `315,0,0,7971,0,0,0.09,0,0,0,2.5,-12,1.25` | 先頭が `time`（ms）、続く 12 列がデータ列 |

列の対応順:

```
turn, speed, battery, angleL, angleR, bright, Kp, Ki, Kd, roll, yaw, pitch
```
...
- 文字コード: UTF-8
- 区切り: 1 レコードごとに改行（`\n` 推奨）を付ける
- 内容: [受信データ形式](#受信データ形式) に準拠（12 データ列、または `time` 付き 13 列）

送信例:

```text
315,0,0,7971,120,80,0.09,0,0,0,2.5,-12.0,1.25
318,0,0,7970,121,81,0.10,0,0,0,2.6,-11.8,1.3
```
...
│ [turn] [speed] [battery] [roll] [yaw] [pitch] ...  ← 系列チェックボックス          │
├────────────────────────────────────────────────────────────────────────────┤
│                                                                            │
│              Plotly 折れ線グラフ（凡例付き）                               │
│                                                                            │
