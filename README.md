...
Bluetooth 接続中に自動保存される CSV は、LogAnalyzer 2015 等で使われていた **レガシー形式** と同じ構造です。`#` で始まる行はコメントとして扱われ、データ行はカンマ区切りの数値列です。

##### ファイル構成

| 行 | 内容 |
|----|------|
| 1 行目 | `# THRESHOLD= ...` — しきい値（Bluetooth 自動保存時は `0.000000`） |
| 2 行目 | `# Speed = ...; Proportional = ...; Integral = ...` — パラメータ（自動保存時はすべて `0.000000`） |
| 3 行目 | `# time, turn, speed, battery, angleL, angleR, bright, gyro, Kp, Ki, Kd` — データ列名 |
| 4 行目以降 | データ行（ヘッダー行はありません） |

##### データ列

| 列名 | 内容 |
|------|------|
| `time` | 接続開始からの経過時間（ms）。整数値の場合は小数点なし |
| `turn` | 旋回量 |
| `speed` | 速度 |
| `battery` | バッテリー電圧 |
| `angleL` | 左モーター角度 |
| `angleR` | 右モーター角度 |
| `bright` | 明るさ |
| `gyro` | ジャイロ値（Bluetooth 受信時は、グラフ描画に使用した数値を記録） |
| `Kp`, `Ki`, `Kd` | PID パラメータ |

Bluetooth から **1 つの数値だけ** 受信する場合、`gyro` 列にその値を書き込み、他の数値列は `0.000000` として記録します。数値に変換できない行も CSV には追記されますが、その行の `gyro` は `0.000000` となり、グラフには反映されません。
...
0,0.000000,0.000000,0.000000,0.000000,0.000000,0.000000,23.500000,0.000000,0.000000,0.000000
15,0.000000,0.000000,0.000000,0.000000,0.000000,0.000000,24.100000,0.000000,0.000000,0.000000
```
...
| 旧 LogAnalyzer2 形式 | `parsed_value` のみ ON |
